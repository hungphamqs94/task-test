...
contract NFTMarket is ReentrancyGuard {
    ...
    function getMarketItems() public view returns(uint[] memory){
        ...
        uint[] memory marketItems = new uint[](unsoldItemCount);
        for (uint256 i = 0; i < itemCount; i++) {
            if (idToMarketItem[i + 1].owner == address(0)) {
                uint256 currentId = idToMarketItem[i + 1].itemId;
                marketItems[currentIndex] = currentId;
                currentIndex += 1;
            }
        }
    ...
    function getItemFollowId(uint256 id) public view returns(uint, address, uint, address, address, string memory, uint, bool){
        MarketItem memory item = idToMarketItem[id];
        return (item.itemId, item.nftContract, item.tokenId, item.seller, item.owner, item.category, item.price, item.isSold);
    }
    
    function fetchPurchasedNFTs() public view returns (uint[] memory) {
        ...
        uint[] memory marketItems = new uint[](itemCount);
        for (uint256 i=0; i< totalItemCount; i++) {
            if (idToMarketItem[i+1].owner == msg.sender) {
                uint256 currentId = idToMarketItem[i+1].itemId;
                marketItems[currentIndex] = currentId;
    ...
    function fetchCreateNFTs() public view returns (uint[] memory) {
        uint256 totalItemCount = _itemIds.current();
        uint256 itemCount = 0;
        uint256 currentIndex = 0;
